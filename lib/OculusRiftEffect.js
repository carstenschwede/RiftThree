/**
 * Based on troffmo5 / http://github.com/troffmo5
 *
 *
...
THREE.OculusRiftEffect = function ( renderer, options ) {
	...
	if (options.scale) {
		HMD.vScreenSize*=options.scale;
	}

	options.emptyColor = options.emptyColor || window.getComputedStyle(document.body).backgroundColor.replace(/ /g,"");
	// Perspective camera
	...
	var emptyColor = new THREE.Color(options.emptyColor);
	...
	this.render = function ( scene, camera ) {
		...
		if (options.preLeft) {
			for(var i=0,l=options.preLeft.length;i<l;i++) {
				options.preLeft[i]();
			}
		}
		...
		if (options.uniforms) {
			for(var i=0;i<options.uniforms.length;i++) {
				options.uniforms[i].left.value = 1.0;
			}
		}
		...
		// Render right
		if (options.preRight) {
			for(var i=0,l=options.preRight.length;i<l;i++) {
				options.preRight[i]();
			}
		}
		...
		if (options.uniforms) {
			for(var i=0;i<options.uniforms.length;i++) {
				options.uniforms[i].left.value = 0.0;
			}
		}
